...
class HomeController extends Controller
{
    ...
    public function show($id, Request $request)
    {
        ...
        if ($request->ajax()) {
            // Trả về HTML cho phần bình luận và phân trang
            return response()->json([
                'comments' => view('comments-list', compact('comments'))->render(),
                'pagination' => view('pagination-links', ['comments' => $comments])->render()
            ]);
        }
        $template = 'blog.index';
        return view('blog.home', compact('categories','post','formattedDate','                'pagination' => view('pagination-links', ['comments' => $comments])->render()
', 'template'));
    }
