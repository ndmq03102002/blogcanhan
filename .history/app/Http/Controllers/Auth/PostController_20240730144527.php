...
class PostController extends Controller
{
    ...
    public function search(Request $request){
        ...
        $posts = Category::with('children')->defaultOrder()->get();
        // Gọi phương thức filter từ service
        $categories = $this->postService->filter($keyword, $catId, $perPage);
        $config['seo'] = config('apps.post');
        $template = "post.index";  
        return view("dashboard.layout", compact("template", "config","categories","rootCategories" ,"keyword", "catId", "perPage","posts"));
    }
