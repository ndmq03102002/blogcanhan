...
class CategoryController extends Controller
{
    ...
    public function search(Request $request){
        ...
        $catId = $request->input('cat_catalogue_id');
        $perPage = $request->input('perpage');
        $categories = Category::whereNull('parent_id')->get()->paginate(10);
        // Gọi phương thức filter từ service
        $cats = $this->catService->filter($keyword, $catId, $perPage);

        $config['seo'] = config('apps.user');
        $template = "category.index";  

        return view("dashboard.layout", compact("template", "config", "cats","categories" ,"keyword", "catId", "perPage"));
    }
