...
@php
    $url = ($config['method'] == 'create') ? route('cat.store') : route('user.update',$user->id);
@endphp
                ...
                <div class="ibox">
                    <div class="ibox-content">
                        <div class="row mb15">
                            ...
                            <div class="col-lg-6">
                                <div class="form-row">
                                    <label for="is_parent" class="control-label text-left">Là danh mục gốc? <span class="text-danger">(*)</span></label>
                                    <input 
                                        type="checkbox"
                                        name="is_parent"
                                        id="is_parent"
                                        value="1"
                                        {{ old('is_parent', isset($category) ? ($category->parent_id === null) : false) ? 'checked' : '' }}
                                        onchange="toggleParentField(this)"
                                    >
                                    @if($errors->has('is_parent'))
                                        <p class="error-message">* {{ $errors->first('is_parent') }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
