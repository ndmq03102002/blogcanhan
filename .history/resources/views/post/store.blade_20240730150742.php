                            ...
                            <div class="col-lg-12 mb15">
                                <div class="form-row">
                                    ...
                                    <select name="category_id[]" id="category" class="form-control select2" multiple="multiple">
                                        @foreach($cats as $cat)
                                        <option value="{{ $cat->id }}" {{ in_array($cat->id, old('category_id', $post->category_id ?? [])) ? 'selected' : '' }}>
                                            {{ $cat->name }}
                                        </option>
...
<script type="module">
    ...
    ClassicEditor
        .create( document.querySelector( '#editor' ), {
            plugins: [ Essentials, Bold, Clipboard, Italic, Font, Paragraph, Image, ImageInsert , AutoImage , ImageToolbar, ImageCaption, ImageStyle, ImageResize, LinkImage,Alignment],
            toolbar: {
                items: [
